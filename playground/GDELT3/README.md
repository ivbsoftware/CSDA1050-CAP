...
## Introduction
This report is describing how to retrieve, process and present online news data related to Toronto and surrounding area (GTA). The main source of information is [The GDELT Project](https://www.gdeltproject.org/) which is a real-time network diagram and database of global human society for open research. GDELT Project monitors the world's broadcast, print, and web news from nearly every corner of every country in over 100 languages and identifies the people, locations, organizations, themes, sources, emotions, counts, quotes, images and events driving our global society every second of every day.
...
Clustering news articles using Doc2Vec model was proven successful by [Rik Nijessen](https://towardsdatascience.com/automatic-topic-clustering-using-doc2vec-e1cea88449c). Unfortunately there no training data or the model are available to reproduce the research.
...
2. Find pre-trained or train new Doc2Vec model.
3. Infer Doc2Vec vectors for each downloaded GTA news article using the model.
4. Cluster GTA news articles.
5. Design algorithm and extract meaningful cluster descriptions from the available data.
6. Prepare the visualization(s).
...
Download, clean and tokenize the Huffington Post news corpus. Parameters of this script are start line number, end line number (-1 if to the end) and suffix of the file generated. This is in case multiple processes are needed to run to speed up the tokenizing.

```python
python prepare_train_corpus.py 0 -1 0 
```
### Training Doc2Vec model
 Training process is done the [this Python code](https://github.com/ivbsoftware/CSDA1050-CAP/blob/master/playground/GDELT3/gta-news/doc2vec/train_doc2vec.py). The script reads __data/news-dataset.txt__ containing ~187K documents and creates Doc2Vec __news-dataset-huffington.model__ file. This is how it should be run from the command line:
...
It contains the following artifacts:
 ...
 - __News_Category_Dataset_v2.json__ is a list of 200K links of news articles from Huffington Post site taken from [Kaggle News Category Dataset](https://www.kaggle.com/rmisra/news-category-dataset).
...
```
This is what it generates if works properly:
```bash
>>> Test 1:
...
The script below extracts/updates GDELT links related to GTA area. The script runs 60 days back, skipping previously downloaded and processed files. Can be run daily to update with latest data.
...
Loads files created on step 5 and generates 30 clusters for 60 days of the 9600 news articles. K-means clustering takes 2 hours on double Xeon workstation.
...
The algorithm pushes the circles apart and at the same time pulls them together to some kind of 'gravitational line' in the middle. This allowed the circles to pack in the center of the canvas keeping the pack shape proportional to canvas rectangle. In the first frame the circles placed on a spiral curve, from the biggest to smallest. The circles then start sorting their positions in relation to each other until movements settle after about 100-800 iterations. The algorithm was applied to generate circle packs for each day (anchor frames). In addition the algorithm generated 9 transitional frames between the anchor frames, 891 frames in total. To keep the positions consistent and movements fluent from frame to frame, each frame was given the previous frame circle positions as starting points, then circle diameters were corrected to reflect the new frame. Then the iteration process would re-pack the circles iteratively. In the movie demonstrating this process (link below) each anchor frame shows for 1 second, the 9 transitions are blended in one half-second transition.
...
At the next step ["GTA News 50 Clusters Chart Using Packed Circles Rendering v.2"](https://github.com/ivbsoftware/CSDA1050-CAP/blob/master/playground/GDELT3/notebooks/GTA%20News%2090%20Days%2050%20Clusters%20WordCloud%20v3.ipynb) notebook generates a combined daily WordCloud bubbles. All 50 clusters are represented by circled WordClouds of the size proportional to number of articles articles. Generation of all 891 slides takes about 10 minutes. The slides were assembled into the slide show of 4K video quality using [ProShow](http://www.photodex.com/proshow/producer) software by PhotoDex and uploaded to YouTube:
 
 [![IMAGE ALT TEXT HERE](https://img.youtube.com/vi/I4tF6VbPuH0/0.jpg)](https://www.youtube.com/watch?v=I4tF6VbPuH0)
 
## Conclusion
The described approach to presenting dynamic flow of news was proven to be feasible. The resulting presentation is informative. Some improvements could be done to the algorithm of circle packing to make it smoother. 

## Next Steps
The following step could taken to transform the presented approach to the production pipeline:

 1. Convert the packing algorithm and Jupyter notebooks to Python scripts.
 2. Develop interactive web application that would dynamically place WordCloud bubble images using JavaScript/HTML5.
