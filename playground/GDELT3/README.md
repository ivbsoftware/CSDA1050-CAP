...
The algorithm pushes the circles apart but pulls them together to some kind of 'gravitational line' in the middle. This allowed the circles to pack toward the center of the canvas keeping the shape proportional to the containing rectangle. In the first frame the circles placed on spiral curve starting from the biggest. The circles then start sorting their positions other until movements settle after about 100-800 iterations. The algorithm was applied to generate circle packs for each day (anchor frames). In addition the algorithm generated 9 transitional frames between those anchor frames (891 frames in total). To keep the positions consistent and movements fluent from frame to frame, each frame was given the previous frame circle positions as starting points, then circle diameters were corrected to reflect the new frame. Then the iteration process would re-pack the circles iteratively. In the movie demonstrating this process (link below) each anchor frame shows for 1 second, the 9 transitions are blended in 0.5 second transition.
